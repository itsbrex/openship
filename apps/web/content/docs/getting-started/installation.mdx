...
If you installed the CLI (the install script or npm), start Openship with a single command:

```bash
openship up
```

This installs Openship as a **background service that starts on boot and restarts itself if it crashes** —
