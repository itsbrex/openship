...
port `3001`. On a local box, access over loopback needs no token.
...
This checks GitHub releases (not your Openship API) and re-installs the global package with your package
manager. After it finishes, restart the server so it runs the new version.
...
<Callout title="Restart after updating">
`openship update` swaps out the installed files but does not restart a running server. Run `openship up`
again afterward to pick up the new version.
</Callout>
