...
<Callout title="Both conditions are required" type="warn">
1. **Opt-in.** The instance's auth mode must be `none`. This is the default for the desktop app; on any
   other deployment it also requires the operator to set `OPENSHIP_ALLOW_ZERO_AUTH=true`.
2. **Loopback only.** The request must come from a loopback address (`127.0.0.1` / `::1`), read from the
...
- **`none`** — zero-auth loopback, described above. Default for desktop.
- **`local`** — standard login required (email + password via [Better Auth](https://www.better-auth.com/)).
  Default for a fresh self-hosted install.
