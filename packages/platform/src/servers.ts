...
import {
  ...
  ServerInstallSessionSchema,
  InstallServerComponentsInputSchema,
  type DeploymentEvent,
  type ServerInstallSession,
  type ServerInstallSessionInput,
  type ServerInstallResponseInput,
  type InstallServerComponentsInput,
...
export interface ServerDependencies {
  ...
  networks?: {
    events(
      ...
      kind: "preparation" | "operation" | "overview" | "runtime" | "storage",
      id?: string,
      signal?: AbortSignal,
...
export type PlatformServerOperations = PlatformScopedOperations<typeof ServerCollectionSchemas> &
  PlatformResourceOperations<typeof ServerResourceSchemas> & {
    openClusterStorageEvents(
      ctx: ExecutionContext,
      id: string,
      options?: { signal?: AbortSignal },
    ): Promise<OperationResult<AsyncIterable<DeploymentEvent>>>;
    ...
    ): Promise<OperationResult<ServerInstallSession>>;
    respondToInstall(
      ctx: ExecutionContext,
      input: ServerInstallResponseInput,
...
export function createServerOperations(
...
): PlatformServerOperations {
  function installations() {
    if (!deps?.installations)
      throw new AppError("Server installations are not configured", 501, "CAPABILITY_UNAVAILABLE");
  ...
  async function session(ctx: ExecutionContext, command?: ServerInstallSessionInput) {
    ...
    if (!Value.Check(ServerInstallSessionSchema, data))
      throw new AppError(
        "Invalid installation session response",
        500,
        "INVALID_OPERATION_RESPONSE",
      );
    const context = await authorization.authorize(ctx, {
      resourceType: "server",
      resourceId: data.active ? data.serverId : "*",
      action: data.active ? "admin" : "read",
    });
    return { context, data };
  }
  async function* authorizedEvents(
    ctx: ExecutionContext,
    id: string,
    action: "read" | "write" | "admin",
    source: AsyncIterable<DeploymentEvent>,
  ) {
    for await (const event of source) {
      await authorization.authorize(ctx, {
        ...
        resourceId: id,
        action,
        ...(id === "*" ? { scope: "all" as const } : {}),
      });
      yield event;
    }
  }
  async function containerStream(
    ctx: ExecutionContext,
    value: string,
    command: unknown,
    options: { signal?: AbortSignal },
    ...
    const input = parseInput(
      apply ? ApplyServerContainerInputSchema : ServerContainerInputSchema,
      command,
    );
    ...
    if (!deps?.containers)
      throw new AppError("Managed containers are not configured", 501, "CAPABILITY_UNAVAILABLE");
    const source = await (apply ? deps.containers.start : deps.containers.events)(
      context,
      id,
      input,
      options.signal,
  ...
  async function networkStream(
    ...
    if (!deps?.networks)
      throw new AppError("Network progress is not configured", 501, "CAPABILITY_UNAVAILABLE");
  ...
  return Object.freeze({
    ...
    openManagedNetworkPreparationEvents: (ctx, id, options = {}) =>
      networkStream(ctx, "preparation", id, options),
    openClusterStorageEvents: (ctx, id, options = {}) => networkStream(ctx, "storage", id, options),
    openClusterRuntimeEvents: (ctx, id, options = {}) => networkStream(ctx, "runtime", id, options),
    openManagedNetworkOperationEvents: (ctx, id, options = {}) =>
      networkStream(ctx, "operation", id, options),
    openClusterEvents: (ctx, options = {}) => networkStream(ctx, "overview", undefined, options),
    openContainerApplyStream: (ctx, id, input, options = {}) =>
      containerStream(ctx, id, input, options, true),
    openContainerApplyEvents: (ctx, id, input, options = {}) =>
      containerStream(ctx, id, input, options, false),
    ...
    async respondToInstall(ctx, command) {
      ...
      return {
        ...
        data: await installations().respond(result.context, result.data.sessionId, input.action),
      };
    },
    ...
    async openInstallEvents(ctx, input, options = {}) {
      ...
      return {
        context: result.context,
        data: authorizedEvents(result.context, result.data.serverId, "admin", source),
      };
    },
